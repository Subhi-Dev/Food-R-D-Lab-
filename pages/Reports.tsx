...
const Reports: React.FC = () => {
  ...
  const filteredReports = MOCK_LAB_REPORTS.filter(r => 
    filter === 'All' || 
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-4xl font-bold text-gray-900 dark:text-white tracking-tight">Lab Reports</h1>
          <p className="text-gray-500 dark:text-slate-400 mt-2 font-medium">QC Analysis & Performance Metrics</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 auto-rows-min">
        ...
        <div className="col-span-1 md:col-span-2 bg-[#F0FDF4] dark:bg-emerald-900/10 dark:border dark:border-emerald-500/20 rounded-[2.5rem] p-8 flex flex-col justify-between min-h-[300px] relative overflow-hidden group">
             <div className="relative z-10 flex justify-between items-start">
                <div>
                    <h3 className="text-emerald-900 dark:text-emerald-100 font-bold text-lg">Pass Rate</h3>
                    <p className="text-emerald-700/60 dark:text-emerald-200/60 text-sm font-medium">Last 30 Days</p>
                </div>
             ...
             <div className="relative z-10 mt-auto">
                <span className="text-6xl font-bold text-emerald-900 dark:text-emerald-50 tracking-tighter">94%</span>
                <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-emerald-800/60 dark:text-emerald-200/50 text-sm font-medium">vs last month</span>
                </div>
             </div>
            ...
            <div className="h-32 w-full">
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={nutrientData}>
                        ...
                        <Tooltip contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }} cursor={false}/>
                        <Area type="monotone" dataKey="retention" stroke="#3b82f6" strokeWidth={3} fillOpacity={1} fill="url(#colorRetention)" />
                    </AreaChart>
                </ResponsiveContainer>
            </div>
            <p className="text-xs text-center text-gray-400 dark:text-slate-500 font-medium mt-2">Retention over processing time</p>
        </div>
                    ...
                    <PieChart>
                        ...
                        >
                            {textureData.map((entry, index) => (
                                <Cell key={`cell-${index}`} fill={TEXTURE_COLORS[index % TEXTURE_COLORS.length]} />
                            ))}
                        </Pie>
        ...
        <div className="col-span-1 md:col-span-4 grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4">
             {filteredReports.map((report) => (
                 <div key={report.id} className="group bg-white dark:bg-slate-900 rounded-[2rem] p-6 border border-gray-100 dark:border-slate-800 shadow-sm hover:shadow-lg hover:border-gray-200 dark:hover:border-slate-700 transition-all cursor-pointer flex items-center gap-6">
                     <div className={`w-16 h-16 rounded-[1.2rem] flex items-center justify-center text-xl font-bold flex-shrink-0 ${
                         report.status === 'Approved' ? 'bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400' :
                         report.status === 'Failed' ? 'bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400' : 'bg-orange-50 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400'
                     }`}>
                        {report.status === 'Approved' ? 'A+' : report.status === 'Failed' ? 'F' : '-'}
                     </div>
                     
                     <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start">
                            <h4 className="font-bold text-gray-900 dark:text-white text-lg truncate group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{report.projectName}</h4>
                            <span className="text-xs text-gray-400 dark:text-slate-500 font-mono">{report.lotNumber}</span>
                        </div>
                        ...
                        <div className="flex gap-2 mt-3">
                            {report.results.slice(0, 2).map((res, i) => (
                                <span key={i} className="px-2 py-1 bg-gray-50 dark:bg-slate-800 rounded-lg text-xs font-medium text-gray-600 dark:text-slate-300 border border-gray-100 dark:border-slate-700">
                                    {res.parameter}: <span className="text-gray-900 dark:text-white font-bold">{res.actualValue}</span>
                                </span>
                            ))}
                        </div>
                     </div>
