...
import { MOCK_CALENDAR_EVENTS, MOCK_AGENDA } from '../constants';
...
const Schedule: React.FC = () => {
  ...
  const getEventStyle = (title: string, type: string) => {
    ...
    switch(type) {
        case 'monitoring': return 'bg-blue-100 dark:bg-blue-900/30 text-blue-900 dark:text-blue-100 border-blue-200 dark:border-blue-800/50';
        case 'panel': return 'bg-orange-100 dark:bg-orange-900/30 text-orange-900 dark:text-orange-100 border-orange-200 dark:border-orange-800/50';
        default: return 'bg-gray-100 dark:bg-slate-800 text-gray-900 dark:text-gray-100 border-gray-200 dark:border-slate-700';
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
            <h1 className="text-4xl font-bold text-gray-900 dark:text-white tracking-tight">Schedule</h1>
            <p className="text-gray-500 dark:text-slate-400 mt-2 font-medium">Lab rotation & Equipment reservation</p>
        </div>
        ...
        <div className="flex items-center gap-4 bg-white dark:bg-slate-900 p-1.5 rounded-full shadow-sm border border-gray-100 dark:border-slate-800">
            ...
            <div className="flex items-center gap-2 px-2">
                ...
                <span className="text-sm font-bold text-gray-900 dark:text-white">Oct 23 - 29, 2023</span>
            </div>
            <button className="p-2 hover:bg-gray-50 dark:hover:bg-slate-800 rounded-full text-gray-500 dark:text-slate-400">
          ...
          <div className="flex-1 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-gray-200 dark:border-slate-800 shadow-sm p-8 min-h-[600px]">
                ...
                <div className="grid grid-cols-5 mb-8">
                    {DAYS.map((day, i) => (
                        <div key={day} className="text-center group cursor-pointer">
                             <span className="block text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest mb-2 group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors">{day}</span>
                             <div className={`w-12 h-12 mx-auto rounded-2xl flex items-center justify-center text-xl font-bold transition-all ${
                                 i === 1 
                ...
                <div className="space-y-4">
                    ...
                    {[9, 10, 11, 12, 13, 14, 15, 16].map((hour) => {
                        ...
                        const hourEvents = MOCK_CALENDAR_EVENTS.filter(e => Math.floor(e.startHour) === hour);
                                        ...
                                        >
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-xs opacity-70 font-medium">{ev.subTitle || 'Lab 4 - Workstation'}</p>
                                                </div>
                                            </div>
          ...
          <div className="w-80 space-y-6 hidden xl:block">
               ...
               <div className="bg-[#FFF7ED] dark:bg-orange-900/10 dark:border dark:border-orange-500/20 rounded-[2.5rem] p-6 relative overflow-hidden">
                    <div className="relative z-10">
                        <h3 className="text-orange-900 dark:text-orange-100 font-bold text-lg mb-1">Today's Focus</h3>
                        <p className="text-orange-800/60 dark:text-orange-200/60 text-xs font-medium mb-6">4 tasks remaining</p>
                        
                        <div className="space-y-3">
                            {MOCK_AGENDA.slice(0, 3).map((task, i) => (
                                <div key={i} className="bg-white/60 dark:bg-white/5 backdrop-blur-sm p-3 rounded-2xl flex items-center gap-3">
                                    <div className={`w-3 h-3 rounded-full border-2 ${task.status === 'done' ? 'bg-green-500 border-green-500' : 'bg-transparent border-orange-300 dark:border-orange-500/50'}`}></div>
                                    <span className={`text-sm font-bold ${task.status === 'done' ? 'text-gray-400 dark:text-slate-500 line-through' : 'text-orange-900 dark:text-orange-100'}`}>{task.title}</span>
                                </div>
                            ))}
                        </div>
                    </div>
               ...
               <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-gray-200 dark:border-slate-800 p-6">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="font-bold text-gray-900 dark:text-white">Up Next</h3>
                        <MoreVertical size={16} className="text-gray-400 dark:text-slate-500" />
                    </div>
                    <div className="flex items-start gap-4">
                        <div className="flex flex-col items-center gap-1">
                            <span className="text-xs font-bold text-gray-400 dark:text-slate-500">WED</span>
                        ...
                        <div className="flex-1 bg-gray-50 dark:bg-slate-800 rounded-2xl p-4">
                            <h4 className="font-bold text-sm text-gray-900 dark:text-white">Sensory Panel</h4>
