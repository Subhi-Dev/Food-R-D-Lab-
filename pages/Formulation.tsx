...
import { RecipePhase, RecipeStep, PhaseColor, StepType } from '../types'
...
const Formulation: React.FC = () => {
  ...
  // Initialize Phases from project data
  useEffect(() => {
    if (project && !initialized) {
      if (project.phases && project.phases.length > 0) {
        setPhases(project.phases as RecipePhase[])
      } else {
        // Initialize Default "Weighing Phase" from ingredients if no phases exist
        const initialSteps: RecipeStep[] = project.ingredients.map((ing) => ({
  ...
  const handleSave = async () => {
    if (project) {
      await updateProjectMutation({ id: project._id, phases: phases as any })
    }
  }
  ...
  const reorderSteps = (phaseId: string, newSteps: RecipeStep[]) => {
    setPhases(
      phases.map((p) => (p.id === phaseId ? { ...p, steps: newSteps } : p))
    )
  }
    ...
    <div className="space-y-8 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center space-x-4">
          <Link
            to="/"
            className="p-3 hover:bg-white dark:hover:bg-[#1e293b] rounded-full transition-colors border border-transparent hover:border-gray-200 dark:hover:border-slate-700"
          >
            <ChevronLeft
              size={24}
          ...
          <div>
            <div className="flex items-center space-x-3 mb-1">
              ...
              <span
                className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${STATUS_COLORS[project.status]}`}
              >
                v{project.version}
              </span>
            </div>
            <p className="text-sm text-gray-500 dark:text-slate-400">
              Formulation Engine • Last edited by {project.lead}
            </p>
          </div>
        </div>
        <button
...
const PhaseContainer: React.FC<{
  ...
  return (
    <div
      className={`rounded-[2.5rem] p-6 md:p-8 ${theme.bg} ${theme.darkBg} border ${theme.border} ${theme.darkBorder} transition-colors`}
    >
      {/* Phase Header */}
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1 space-y-4">
            ...
            <div className="bg-gray-50 dark:bg-[#0f172a] rounded-2xl p-4">
              {step.type === 'weighing' && (
                <WeighingControl step={step} onUpdate={onUpdate} />
              )}
              {step.type === 'timer' && (
                <TimerControl step={step} onUpdate={onUpdate} />
              )}
              {step.type === 'process' && (
                <textarea
                  value={step.notes || ''}
  ...
  return (
    <div className="flex flex-wrap items-center gap-4">
      <div className="flex-1 min-w-[140px]">
        <label className="block text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-1">
          Target
        </label>
        <div className="flex items-center gap-2">
          <input
            ...
            onChange={(e) =>
              onUpdate({ expectedWeight: parseFloat(e.target.value) })
            }
      ...
      <div className="flex-1 min-w-[140px]">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            onChange={(e) =>
              onUpdate({ actualWeight: parseFloat(e.target.value) })
            }
            className={`w-24 bg-white dark:bg-slate-700 rounded-lg px-3 py-2 text-sm font-bold border focus:outline-none focus:ring-2 focus:ring-blue-500 ${
              step.expectedWeight &&
              step.actualWeight &&
              Math.abs(step.actualWeight - step.expectedWeight) < 5
  ...
  return (
    <div className="flex items-center gap-6">
      ...
      <div className="ml-auto flex items-center gap-2">
        <label className="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase">
          Set Duration (s)
        </label>
        <input
          type="number"
