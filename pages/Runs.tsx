...
const Runs: React.FC = () => {
  ...
  const handleStartRun = (project: Project) => {
    ...
    addNotification(
      'Run Started',
      `Initialized batch ${code} for ${project.name}`,
      'info'
    )
  }
  ...
  const finishRun = () => {
    ...
    const newRecord: RunRecord = {
      id: `run-${Date.now()}`,
      projectId: (selectedProject as any)._id || selectedProject.id || '',
    ...
    addNotification(
      'Batch Completed',
      `Successfully logged run ${batchCode}.`,
      'success'
    )
  }
  ...
  const getValidationState = () => {
    if (!activeStep)
      return { isValid: false, message: '', color: '', bgColor: '', range: '' }
      ...
      <div className="space-y-12 max-w-6xl mx-auto pb-12">
        <div className="px-4 md:px-0">
          ...
          <p className="text-gray-500 dark:text-slate-400 mt-2 font-medium">
            Initialize a new production run or view session history.
          </p>
        </div>
          ...
          <div className="xl:col-span-2 space-y-8">
            ...
            <div className="bg-white dark:bg-[#1e293b] rounded-[2rem] p-8 shadow-lg border border-gray-100 dark:border-slate-800">
              ...
              <div className="relative group">
                <select
                  ...
                  onChange={(e) => {
                    const p = projects.find(
                      (proj: any) => (proj._id || proj.id) === e.target.value
                    )
                ...
                <div
                  className={`absolute ${isRTL ? 'left-6' : 'right-6'} top-1/2 -translate-y-1/2 pointer-events-none text-gray-400 group-hover:text-blue-500 transition-colors`}
                >
                  <ChevronDown size={28} />
                </div>
              </div>
            ...
            {/* Quick Select Grid */}
            <div>
              <h3 className="text-sm font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-6 px-1">
                Quick Select
              </h3>
                  ...
                  >
                    <div className="relative z-10">
                      <div className="flex justify-between items-start mb-4">
                        <span
                          className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide ${STATUS_COLORS[project.status]}`}
                        >
                          {project.status}
                        </span>
                        <div className="p-2 bg-gray-100 dark:bg-slate-700 rounded-full text-gray-400 dark:text-slate-400 group-hover:bg-blue-600 group-hover:text-white transition-colors">
                          <Play
                            size={20}
                            fill="currentColor"
                            className={isRTL ? 'transform -scale-x-100' : ''}
                          />
                        </div>
                      </div>
              ...
              {runHistory.length === 0 ? (
                ...
                <div className="space-y-4">
                  {runHistory.map((run) => (
                    <div
                      key={run.id}
                      className="p-4 rounded-2xl bg-gray-50 dark:bg-slate-900/50 hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors border border-transparent dark:border-slate-800/50"
                    >
                      ...
                      <h4 className="font-bold text-gray-900 dark:text-slate-200 text-sm mb-1">
                        {run.projectName}
                      </h4>
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-6 animate-in zoom-in-95 duration-500 px-4">
        ...
        <p className="text-xl text-gray-500 dark:text-slate-400 max-w-lg">
          The formulation run for{' '}
          <span className="font-bold text-gray-900 dark:text-white">
            {selectedProject?.name}
          </span>{' '}
          has been logged to history.
        </p>
  ...
  const totalSteps =
    phases.reduce((acc, phase) => acc + phase.steps.length, 0) || 0
  ...
  const progressPercent =
    totalSteps > 0 ? (currentGlobalStep / totalSteps) * 100 : 0
          ...
          >
            ...
            <div className="flex items-start justify-between mb-8">
              <div className="flex-1">
                <span
                  className={`inline-block px-4 py-2 rounded-xl text-sm font-bold uppercase tracking-wider mb-4 ${
            ...
            <div className="flex-1 flex flex-col justify-center py-8">
              ...
              {activeStep?.type === 'weighing' && (
                <div className="flex flex-col lg:flex-row items-center gap-12">
                  ...
                  <div className="flex-1 space-y-2 text-center lg:text-left rtl:lg:text-right w-full">
                    <label className="block text-sm font-bold uppercase tracking-wider text-gray-400 dark:text-slate-500">
                      {t('targetWeight')}
                    </label>
                    <div className="text-5xl md:text-6xl font-bold text-gray-900 dark:text-white font-mono tracking-tight">
                      ...
                      <span className="text-3xl md:text-4xl text-gray-400 ml-2">
                        g
                      </span>
                    </div>
                  ...
                  <div className="flex-1 w-full space-y-4">
                    ...
                    <div className="relative">
                      ...
                      />
                      <div
                        className={`absolute ${isRTL ? 'left-6' : 'right-6'} top-1/2 -translate-y-1/2 pointer-events-none`}
                      >
                        {actualWeightInput &&
                          (validation.isValid ? (
                            <CheckCircle2
                              size={32}
                              className="text-emerald-500 md:w-10 md:h-10"
                            />
                          ) : (
                            <AlertCircle
                              size={32}
                              className="text-rose-500 md:w-10 md:h-10"
                            />
                          ))}
                      </div>
                    </div>
                    ...
                    <div
                      className={`p-4 rounded-2xl flex items-center gap-3 transition-all duration-300 ${validation.bgColor}`}
                    >
                      <div
                        className={`p-2 rounded-full bg-white/50 dark:bg-black/20 ${validation.color}`}
                      >
                        {validation.isValid ? (
              ...
              {activeStep?.type === 'timer' && (
                <div className="space-y-8">
                  ...
                  >
                    <div
                      className={`w-6 h-6 rounded border-2 flex items-center justify-center ${isStepConfirmed ? 'bg-blue-600 border-blue-600' : 'border-gray-300 dark:border-slate-500'}`}
                    >
                      {isStepConfirmed && (
                        <Check size={16} className="text-white" />
                      )}
                    </div>
                    <span className="font-bold text-gray-600 dark:text-slate-300 text-sm md:text-base">
              ...
              {activeStep?.type === 'process' && (
                ...
                >
                  ...
                  <div>
                    <h3
                      className={`text-xl md:text-2xl font-bold ${isStepConfirmed ? 'text-white' : 'text-gray-900 dark:text-white'}`}
                    >
                      Mark as Done
                    </h3>
                    <p
                      className={`text-sm md:text-base mt-1 ${isStepConfirmed ? 'text-blue-100' : 'text-gray-500 dark:text-slate-400'}`}
                    >
