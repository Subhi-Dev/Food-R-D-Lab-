...
const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  const handleArchive = () => {
    if (onArchive) onArchive(projectId)
    setIsMenuOpen(false)
  }

  return (
    <div
      className={`relative h-[280px] group transition-transform hover:-translate-y-1 hover:shadow-lg dark:hover:shadow-none`}
    >
      {/* Background & Decor - Lower z-index */}
      <div
        className={`absolute inset-0 rounded-[2rem] overflow-hidden pointer-events-none z-0 ${theme.bg} dark:border ${theme.border || 'dark:border-white/10'}`}
      >
        {/* Logical placement for blob: 'end-0' puts it on Right in LTR, Left in RTL (roughly) */}
        <div
          className={`absolute -bottom-10 ${isRTL ? '-left-10' : '-right-10'} w-40 h-40 bg-white/20 dark:bg-white/5 rounded-full blur-2xl`}
        />
      </div>
      ...
      <div className="relative h-full p-7 flex flex-col justify-between z-10">
        ...
        <div className="flex justify-between items-start">
          <div
            className={`px-3 py-1 bg-white/60 dark:bg-white/10 backdrop-blur-sm rounded-full text-xs font-bold uppercase tracking-wide ${theme.sub}`}
          >
                ...
                >
                  <div className="flex flex-col gap-1">
                    <button
                      ...
                      className="flex items-center gap-3 px-3 py-2 text-sm font-medium text-gray-700 dark:text-slate-200 hover:bg-black/5 dark:hover:bg-white/10 rounded-xl transition-colors text-start"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-3 px-3 py-2 text-sm font-medium text-gray-500 dark:text-slate-400 hover:bg-black/5 dark:hover:bg-white/10 rounded-xl transition-colors text-start"
                    >
                      <Archive size={16} /> {t('archiveProject')}
                    </button>
                    <button
                      onClick={handleDelete}
                      className="flex items-center gap-3 px-3 py-2 text-sm font-bold text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors text-start"
                    >
        ...
        <div className="mt-2">
          <h3
            className={`text-2xl font-bold ${theme.text} mb-1 leading-tight line-clamp-2 text-start`}
          >
            {project.name}
          </h3>
          <p
            className={`text-sm font-medium ${theme.sub} opacity-80 text-start`}
          >
            v{project.version} • {project.updatedAt}
          </p>
        </div>
