...
const Inventory: React.FC = () => {
  ...
  const getIconForCategory = (category: string) => {
    switch(category) {
      case 'Stabilizers': return <Grid size={20} />;
      case 'Sweeteners': return <Droplets size={20} />;
      case 'Bases': return <Leaf size={20} />;
      case 'Cultures': return <Sun size={20} />;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-4xl font-bold text-gray-900 dark:text-white tracking-tight">Raw Materials</h1>
          <p className="text-gray-500 dark:text-slate-400 mt-2 font-medium">Manage stock levels and batch expiry</p>
        </div>
        
        <div className="flex items-center gap-4 w-full md:w-auto">
             <div className="relative group flex-1 md:flex-none">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-slate-500 group-focus-within:text-gray-900 dark:group-focus-within:text-white transition-colors" size={20} />
                <input 
                    type="text" 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredItems.map((item, idx) => {
           ...
           return (
            <div key={item.id} className={`${theme.bg} dark:border dark:border-white/5 rounded-[2.5rem] p-7 flex flex-col justify-between h-[320px] relative group transition-transform hover:-translate-y-1 hover:shadow-lg dark:hover:bg-opacity-30`}>
                
                {/* Header */}
                <div className="flex justify-between items-start z-10">
                    <div className={`w-12 h-12 rounded-[1rem] ${theme.icon} flex items-center justify-center`}>
                        {getIconForCategory(item.category)}
                    </div>
                    <button className={`p-2 rounded-full hover:bg-white/40 dark:hover:bg-white/10 transition-colors ${theme.text}`}>
                ...
                <div className="z-10 mt-4">
                    ...
                    <h3 className={`text-2xl font-bold ${theme.text} leading-tight mb-1`}>{item.name}</h3>
                    <p className={`text-sm font-medium ${theme.text} opacity-70 font-mono`}>{item.batchId}</p>
                </div>

                {/* Footer Details */}
                <div className="z-10 mt-auto bg-white/40 dark:bg-black/20 backdrop-blur-sm rounded-[1.5rem] p-4 space-y-3">
                    <div className="flex justify-between items-center">
                        <span className={`text-xs font-bold uppercase tracking-wide ${theme.text} opacity-60`}>Current Stock</span>
                        <span className={`text-lg font-bold ${theme.text}`}>{displayStock}</span>
                    </div>
                    
                    <div className="flex justify-between items-center">
                         <span className={`text-xs font-bold uppercase tracking-wide ${theme.text} opacity-60`}>Expires</span>
                         <span className={`text-sm font-bold ${item.expiryStatus === 'expiring' ? 'text-red-600 dark:text-red-300' : theme.text}`}>
                            {item.expiryDate}
                         </span>
                    </div>
                    ...
                    {item.usedIn.length > 0 && (
                        <div className="pt-2 border-t border-gray-900/5 dark:border-white/10 flex items-center gap-2 overflow-hidden">
                            <span className={`text-[10px] ${theme.text} opacity-60 whitespace-nowrap`}>Used in:</span>
                            <div className="flex gap-1 overflow-x-auto no-scrollbar">
                                {item.usedIn.map((proj, i) => (
                                    <Link key={i} to={`/project/${proj.id}`} className="px-2 py-0.5 bg-white/60 dark:bg-white/10 rounded-full text-[10px] font-medium text-gray-700 dark:text-gray-200 whitespace-nowrap hover:bg-white dark:hover:bg-white/20 transition-colors">
                                        {proj.name}
                                    </Link>
                                ))}
                            </div>
